...
const Banner = () => {
  ...
  return (
    <>
      <section className="h-70 sm:h-110 w-auto rounded-sm shadow relative overflow-hidden">
        <Slider {...settings}>
          {banners.map((el, i) => (
            <img draggable="false" className="h-70 sm:h-110 w-auto object-cover" src={el} alt="banner" key={i} />
          ))}
        </Slider>
      </section>
    </>
