...
const PrimaryDropDownMenu = ({ setTogglePrimaryDropDown, user }) => {
    ...
    return (
        <div className="absolute w-60 -left-24 ml-2 top-9 bg-white shadow-2xl rounded flex-col text-sm" onClick={closebar} onMouseLeave={closebar}>
            ...
            {navs.map((item, i) => {
                ...
                return (
                    <React.Fragment key={i}>
                        {title === "Wishlist" ? (
                            <Link className="pl-3 py-3.5 border-b flex gap-3 items-center hover:bg-gray-50" key={i} to={redirect} >
